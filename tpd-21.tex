...
\begin{itemize}
    ...
    \item checkout repository changes to local (\texttt{traverse/get})
...
track, then we create and commit it:

{\footnotesize
\begin{verbatim}
 > freechains #p2p.md join A2885F..
 > echo "Peer-to-peer networking is..." > p2p.md
 > freechains-vcs #p2p.md commit
 1_4F3EE1..
...
The loop acquires each of the payloads representing the patches and apply them
in order to recreate the current version of the file in the chain.

Since the last edition is clearly wrong, other users in the network will
dislike its commit until it becomes \emph{HIDDEN} in the chain
(Figure~\ref{fig.state}):

{\footnotesize
\begin{verbatim}
 > freechains chain #p2p.md dislike 23_AE3A1B --sign=49A0DD..
 > freechains chain #p2p.md dislike 23_AE3A1B --sign=F481A4..
 > freechains chain #p2p.md dislike 23_AE3A1B --sign=CDA911..
 > freechains-vcs #p2p.md checkout p2p.md
 > cat p2p.md
...
This way, the checkout operation above will apply an empty patch and the
offending line will disappear from the file.
