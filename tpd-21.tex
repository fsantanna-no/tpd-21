...
communications. % that combats abuse.
...
increasingly more centralized in a few companies (e.g. Facebook and Twitter).
...
%
Consensus is key to eradicate Sybils~\cite{p2p.sybil}, which are the major
threats to open forums and decentralized collaborative applications in general:
without consensus, it is not possible, \emph{at the protocol level}, to
distinguish between correct and malicious users.
...
to context of content publishing.
%
...
%
Due to decentralization, posts in timelines form a causal graph with only
...
are forced, at the protocol level, to remove associated payloads, only
...
A dVCS is relevant because
    ...
    (iii) with merges that require human intervention, which we automate.
%
Hence, as a forth contribution, we show how the consensus mechanism can empower
(mostly) conflict-free replicated data types (CRDTs~\cite{p2p.merkle-crdts})
when they do encounter conflicting operations, which we reword as
\emph{quasi-CRDTs}.

In summary, we propose (i) a consensus mechanism based on proof-of-authoring,
(ii) for independent user-generated blockchains, (iii) which supports content
removal while preserving integrity, and (iv) which can automate conflict
resolution in quasi-CRDTs.
%
...
chain.
The protocol also ensures strong eventual consistency, if we consider that the
DAG itself represents the state of the peers~\cite{fed.matrix,p2p.byz}.

The protocol operation is typical of publish-subscribe systems: an author
publishes a post to a chain, and the other users subscribed to the same
chain eventually receive the message by synchronizing DAGs.

The goals of this section are twofold:
    (i)  to depict the chain DAGs as result of the basic protocol operations; and
...
In \FC, the synchronization operation of chain DAGs is explicit, in pairs, and
unidirectional.
...
message ordering within forums across all peers in the network.
...
consensus algorithm needs to execute at the core of the protocol, while peers
synchronize their chain DAGs.
From the perspective of a peer on the receive side, the overview of the
consensus and synchronization mechanism is as follows:
\begin{enumerate}
...
Step (b) is very similar to \emph{Byzantine Causal Broadcast}~\cite{p2p.dag.sync},
which also synchronizes DAGs representing causal messages in the presence of
malicious nodes:
    ...
    (iv)  each message represents an iteration of Step (b).
...
heads to adapt to this limitation.
...
It verifies the following properties:
    ...
    (ii)  Back pointers are not in \emph{BLOCKED} state (discussed above);
...
consensus algorithm should consider it.
...
\item Removes from $S$ the block $B$ in the branch with more reputation
      ...
      This steps differs from Kahn's algorithm, which would take any block from
      $S$.
...
reputation criteria.
The comparison function needs to find the authors that are not in common in
the suffix of the two branches in order to compare the sum of their
reputations.
%
For this reason, the algorithm also needs to track negative, neutral, and
positive blocks according to Table~\ref{fig.rules}.
%
We take advantage of the hard fork rule that fixes the consensus list $L$ when
crossing an activity threshold (e.g., \emph{7 days} or \emph{100 posts}).
We use it as a checkpoint to cache the reputations, which limits the input size
to a fixed sized that does not depend on the sizes of the chains.
...
We build the DAG by traversing the chain directory and save the cache and
current consensus order in an index file.
...
- usenet
    - more machines, less connections
    - less machines, more connections
...
which provides non-trivial datasets with robust merging policies, but which
...
A similar dVCS with a DAG representation and merging policies have been
recently proposed~\cite{p2p.dvcs}.
To resolve what they call \emph{competition conflicts}, they propose to use an
external scoring function, such as users' reputations.
...
The key insight of the mechanism is to use the human authoring ability as a
