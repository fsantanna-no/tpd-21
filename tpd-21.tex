...
\begin{IEEEkeywords}
bitcoin, crdt, dvcs, distributed consensus, peer-to-peer, publish-subscribe, reputation system
\end{IEEEkeywords}}
...
However, due to the decentralization of authority and network infrastructure,
some new challenges arise to deal with malicious users and to enforce overall
state consistency.
...
These mechanisms lead to three-layered CRDT framework:
    a CvRDT at the transport layer,
    a CmRDT relying on commutative operations at the application layer, and
    a CmRDT with arbitrary operations after consensus is applied.
Next, we illustrate this framework through an example of a simple
permissionless DVCS implemented on top of public chains.
...
We apply the three-layered CRDT framework as follows:
