...
publish-subscribe protocol to model content dissemination.
...
open protocols to lock their users in proprietary platforms.
...
collecting our data and ``algorithmizing'' our consumption.
%
...
%illegal content).
At the very core, consensus is key to eradicate Sybil attacks which are the
fundamental threat to decentralized collaborative applications: without
...
As an application example, we also prototyped a permissionless distributed
version control system (DVCS) that relies on consensus to apply automatic
merges.
...
- example is relevant
    - collaborative
    - social consensus
    - eventual consistency (not the whole graph)
