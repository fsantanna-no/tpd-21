...
%
We executed each simulation $4$ times in a conventional desktop PC ($i7$ CPU,
$8GB$ RAM, $512GB$ SSD).
...
The simulated chat chain is $8MB$ in size, which indicates a $10x$ overhead.
...
much longer).
The simulated newsgroup chain is $42MB$ in size, which indicates a $50\%$
overhead.
...
sorts respectively, while for the newsgroup chain, it takes $100s$ and $70ms$.
%
...
of asynchronicity between peers following the local-first principle.
...
%
As expected, the newsgroup should have a lower bookkeeping given the higher
interval between messages.

A few notes about the subjective behavior of chains, which our analysis does
not evaluate:
First, the original users were not aware of the reputation rules, which
otherwise could decrease the required bookkeeping even further.
Second, we assume that users would use the revoke mechanism to combat abusive
content, having no further effects on our evaluation.
Third, we do not evaluate the quality of posts and how they would affect the
reputation of users.
