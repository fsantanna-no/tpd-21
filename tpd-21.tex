...
Figure~\ref{fig.like} illustrates the chain DAG up to the like operation.
...
After at most 12h, these penalties are dismissed and the \nreps{2} are
recovered (rule~\texttt{2}):
    the pioneer goes to \nreps{29} and the new user to \onerep, which sum up to
    the pioneer's original \nreps{30}.

With no additional rules to create \reps, the inital \nreps{30} would
constitute the whole chain economy, just moving between users.
Rule~\ref{1.b} ...


Besides the pioneer rule, the only way to create reputation
...
\caption{
    \texttt{\#forum} DAG: like approved the newbie message into the chain.
}
\label{fig.like}
\end{figure}
...
Note also that nrule{1.a}{pioneer} solves a chicken-and-egg problem in which
