...
Hector Sanjuan, Samuli P oyhtari, Pedro Teixeira, and Ioannis Psaras
0 cites
We study Merkle-DAGs as a transport and persis-
...
Additionally, CRDTs also feature monotonicity.
...
operation-based CRDTs
    - FC is state-based and nodes are operations, better of both worlds
        - exactly once op (not 0 not 2)
        - only holds ops (not full states)
...
sync algorithm???
    - two dags if one contains the other, ok
    - otherwise sync diffs
    - converge, monotonic
    - Merkle-Clock -> Growing Set
