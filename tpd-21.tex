...
In permissionless peer-to-peer systems, it takes a few seconds to generate
thousands of identities as public/private keys and SPAM million of messages in
the system.
...
consensus algorithm to make peer-to-peer public forums practical.

\subsection{Overall Design}

The reputation system introduces tokens named \reps that users can spend to
...
%
However, without restrictions, like operations alone are not satisfactory,
specially considering hypothetical Sybils abusing the system.
...
Authoring takes time and is, to some extent, easy to verify and rate.
...
%
Unlike Bitcoin, forks are not only allowed but encouraged by the local-first
principle.
...
software.
The last behavior is similar to chain reorganization in Bitcoin when a peer
...
The public forums of \FC follow strict rules that track the reputation of users
in order to enforce consensus and support content moderation.
...
Rule~\texttt{1.a} bootstraps a public chain assigning \nreps{30} to the pioneer
...
gradually transferring \reps to other authors, which also transfer to other
authors, expanding the community.
%
...
Other alternatives include building social trust graphs, in which users already
in the community vouch for new users, or imposing economic costs to new
posts, such as solving proof-of-work puzzles.

We propose a mix between the last two approaches: trust on new users and costs
in \reps.
...
Then, the pioneer likes the \emph{newbie} message, which reduces himself to
...
For this reason, rule~\texttt{1.b} awards authors with \onerep 24 hours after
of a new post, stimulating content creation and growing the economy of chains.
...
Therefore, 24 hours after the commands above, the pioneer accumulates
...
provides incentives to spend likes and decentralize the network.
...
On the other hand, \reps are limited to at most \nreps{30} per author and
expire after 90 days (rules~\texttt{4.b} and~\texttt{4.c}).
The scarcity and limits work together towards the quality of the chains.
%
...
However, considering that \reps are scarce, dislikes are more directed to
combat abuse, but not to eliminate divergence of opinion.
...
%
If the author has reputation, its new post is immediately accepted in the
chain.
...
other peers.
However, blocked posts do not reach other users, they will never have the
chance to be welcomed with a like.
A reasonable policy for blocked posts is to hold them in a temporary queue and
retransmit them to a small extent for some visibility in the network.
%
Once accepted, the post becomes part of the chain and can never be removed
again since Merkle DAGs are immutable by design.
Note that blocked posts that become accepted are always be succeeded by a
\emph{like} (Figure~\ref{fig.forum}).
%
If the number of dislikes exceed the threshold, the post payload becomes
hidden, i.e., it is not retransmitted to other peers.
% and should be removed from local storage.
Removing the payload is harmless because the Merkle DAG depends only on the
Furthermore, the DAG itself contains the dislikes that prove the hidden state
to other peers.
