...
The genesis block with height $0$ and hash $42209B...$ depends only on the
arguments given to \emph{join}.
The next block with height $1$ contains the posted message and its hash
$EF5DE3$ depends on its payload and the hash of its previous block, complying
with the Merkle~DAG.
...
Hence, before synchronizing, the others peers should initialize the chain with
the same steps:
...
1_EF5DE3...
$ freechains "$family" get 1_EF5DE3...
Good morning!
\end{verbatim}
}
...
post before recv
see the fork
but no problem b/c operations are not handled by the protocol
only really matter if the protocol needs an order


Note that 


use the generated public key to operate on a public identity chain
The key pairs are generated deterministically from 

local first

Falar sobre @identity
Terminar com problema em #public
