...
DAGs to structure its messages~\cite{33,34}.
...
\caption{
    ...
    (c) If rules (a) and (b) conflict, the merge fails.
}
\label{fig.reps}
\end{figure}

As a counterpoint, suppose that in the same example of Figure~\ref{fig.reps}.A,
users $a$ and $b$ actually abandoned the chain for months and \emph{branch-1}
is legit.
In this case, $a$ and $b$ are the ones malicious trying to take over the chain
by artificially creating an early \emph{branch-2}, even with knowledge about
\emph{branch-1}.
There is even a third option in which both branches are legit but were
disconnected for a long period.
It is impossible to determine.
Nonetheless, the breach that permits an early branch to take over a long active
chain is compromising.
For this reason, the consensus algorithm includes an extra constraint on
merging:
If a branch creates new \reps that reach 50\% of its prefix, then it is
guaranteed that the branch will take priority in merges.
...
%
Figure~\ref{fig.consensus} summarizes the merging algorithm.
