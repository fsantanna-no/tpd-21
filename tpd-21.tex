...
We propose a reputation system and consensus algorithm that work together to
...
content within excess.
A \emph{dislike} operation is a negative feedback that blocks content when
crossing a threshold.
%
Like operations alone are not enough because of Sybils, so \reps must be
subject to some sort of scarcity that demands non-trivial work immune to
automation.
...
but limited by periods of time to prevent excess (e.g., once in a day per
user).
Additionally, likes and dislikes only transfer \reps between users to preserve
scarcity.
%
...
Since posts and like operations are happening concurrently, we need to figure
out how to order them consistently across all peers so that they converge to
the same state.

\begin{figure}[t]
\centering
\includegraphics[width=0.49\textwidth]{reps.png}
\caption{TODO}
\label{fig.reps}
\end{figure}
...
%
Figure~\ref{fig.reps}.A illustrates the reputation criterion, still abstractly,
since we did not discuss the actual rules for content creation and rating.
...
%
After the prefix, the chain forks into two branches.
...
At any point in time, if an operation fails (e.g., a like from a user without
reputation), all remaining blocks are removed from the chain DAG.

Some considerations about merging forks:
...
note that even after a peer sees both branches in
Figure~\ref{fig.reps}.A, 




When merging
, possibly as Sybils of $c$ to generate more content.
