...
%
However, unlimited posts and likes are not satisfactory in the presence of
Sybils.
...
cooperate, which is non-trivial and contradicts their intent.
...
Nonetheless, \emph{branch~2} would take priority because, before the forking
...
\emph{branch~1}.
The ordered list after consensus is only a view of the primary forum DAG
structure, created for accountability purposes.
...
%
Peers that received branches with less reputation first will need to reorder
...
%
Likewise, peers that saw branches with more reputation first just need to put
the other branch in sequence and do not need to recompute anything.
The latter should be the normal behavior expected to happen in the majority of
the network.
%
...
Another possibility is that both branches are legit but became disconnected for
a long period.
It is simply impossible to judge with confidence.
Nonetheless, it is unacceptable that a very old branch affects a long active
chain.
...
algorithm favors this branch in future merges.
...
Better alternatives include (i) building social trust graphs, in which users
...
foundation to model shared data with concurrent updates from collaborative
local-first applications~\cite{p2p.local}.
%
We now introduce a three-layered CRDT scheme to build distributed applications:
    state-based CRDTs (CvRDT) at transport layer,
    operation-based CRDTs (CmRDT) at application layer, and
    CRDTs with arbitrary operations after consensus is applied.

At the transport layer, Merkle~DAG chains are trivial CvRDTs because their
missing parts are exchanged on synchronization to converge to the same
state~\cite{p2p.merkle-crdts}.
%
...
%
CmRDTs have the advantage to store only small updates enough to reconstruct any
version of the data.
...
commutative operations in the third layer.
...
A \emph{checkout} needs to recreate the latest version of the file in the
repository by applying all patches since the genesis block.
...
Since the last commit above is clearly wrong (peer-to-peer networks do scale),
other users in the network will dislike it until the block becomes
\emph{HIDDEN} in the chain (as described in Figure~\ref{fig.state}):
...
\begin{verbatim}
 ...
 The [USENET](#usenet.md), ...  \- not "computing"
 ...
\end{verbatim}
}
...
It only requires the standard \emph{diff \& patch} tool and the basic API of
\FC.
...
DAG between peers, which eventually reach the same state.
...
Even blocked posts (Figure~\ref{fig.state}), which have limited reachability,
are not a concern either.
...
but which applies only in clients, after the messages have been delivered.
...
Users can tag posts, which appear not only in their feeds but also in local
